    ...
    *,
    ...
    with stage_for_run(ctx.agent_id, ctx.sid, ctx.work_dir, engine="codex"):
        ...
        return _run_codex_subprocess(
            ...
            is_local=is_local, log_path=ctx.log_dir / f"{ctx.task_id}.jsonl",
            last_msg_path=ctx.log_dir / f"{ctx.task_id}.codex_last_message.txt", model=model,
...
def _run_codex_subprocess(
    *,
    ...
    req = task_request(
        ...
        log_path=log_path, last_msg_path=last_msg_path, schema=schema)
