...
def claude_code_task(
    ...
    *,
    ...
    the parsed object comes back as ``structured_output``. ``step_ctl``: session
    policy / budget / rotation from the run executor (see ``_common``).
    """
    ...
    with stage_for_run(ctx.agent_id, ctx.sid, ctx.work_dir, engine="claude", agent_md_file=agent_md_file):
        ...
        def run_fn(*, prompt, resume_id, fork, schema, budget, add_dirs, lineage, log_suffix=""):
            return _run_claude_subprocess(
                ...
                is_local=is_local, log_path=ctx.log_dir / f"{ctx.task_id}{log_suffix}.jsonl", model=model,
                resume_store=ctx.resume_store, append_system_prompt_file=append_file, schema=schema,
                agent_id=ctx.agent_id, fork=fork, budget=budget, add_dirs=add_dirs, lineage=lineage)
...
def _run_claude_subprocess(
    *,
    ...
    req = task_request(
        ...
        fork=fork, budget=budget, add_dirs=add_dirs)
