...
How an agent applies this system in a TeleDesign project. The project receives a read-only copy at `_ds/playful/`.
...
<link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Fredoka:wght@500;600;700&family=Nunito:wght@400;600;700;800&family=DM+Mono:wght@400;500&display=swap">
<link rel="stylesheet" href="_ds/playful/tokens.css">
<link rel="stylesheet" href="_ds/playful/components.css">
```
...
- Components: load after React 18.3.1 / ReactDOM / Babel standalone 7.29 (SRI hashes are in any `*.card.html`):
  `<script type="text/babel" data-presets="react" src="_ds/playful/components/actions/Button.jsx"></script>` — each file sets `window.<Name>`.
...
- Import the library: `"imports": { "ds": "_ds/playful/system.lib.pen" }` and bind properties to its variables (`"fill": "$--primary"`, `"cornerRadius": "$--radius-control"`, `"fontFamily": "$--font-ui"`).
...
- Keep one coral CTA per view.
- Use tinted circles behind icons.
- Round everything — no square corners anywhere.
- Avoid: No thin (<400) weights.
- Avoid: No grey-on-grey low-contrast text.
- Avoid: No emoji inside buttons or form labels.
