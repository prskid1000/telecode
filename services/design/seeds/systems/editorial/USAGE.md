...
How an agent applies this system in a TeleDesign project. The project receives a read-only copy at `_ds/editorial/`.
...
<link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Fraunces:wght@400;600;700&family=Source+Serif+4:ital,wght@0,400;0,600;1,400&family=Instrument+Sans:wght@400;500;600&family=IBM+Plex+Mono:wght@400;500;600&display=swap">
<link rel="stylesheet" href="_ds/editorial/tokens.css">
<link rel="stylesheet" href="_ds/editorial/components.css">
```
...
- Components: load after React 18.3.1 / ReactDOM / Babel standalone 7.29 (SRI hashes are in any `*.card.html`):
  `<script type="text/babel" data-presets="react" src="_ds/editorial/components/actions/Button.jsx"></script>` — each file sets `window.<Name>`.
...
- Import the library: `"imports": { "ds": "_ds/editorial/system.lib.pen" }` and bind properties to its variables (`"fill": "$--primary"`, `"cornerRadius": "$--radius-control"`, `"fontFamily": "$--font-ui"`).
- Theme axis `Mode` = `Light` (default) / `Dark`; set `"theme": {"Mode": "Dark"}` on a frame to preview the other theme.
- Instantiate components with `ref` (e.g. `btn-primary-md`, `input-default`, `card-default`, `badge-success`, `tabs-underline`, `dialog-default`, `table-default`, `nav-horizontal`) and change text through `descendants` (`{"btn-primary-md-label": {"content": "Save"}}`). Component names follow `Component/Variant[/Size]`.
...
- Keep body text at `--text-base` (18px) with a 62-72ch measure.
- Use one oxblood element per viewport as the focal point.
- Prefer rules and whitespace over boxes.
- Avoid: No rounded image corners.
- Avoid: No bright saturated colours or gradients.
- Avoid: No sans-serif headlines.
