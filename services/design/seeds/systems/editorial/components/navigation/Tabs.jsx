/* TeleDesign seed system "Editorial" — Tabs.
...
(() => {
  ...
  function Tabs({ items = [], value, defaultValue, onChange, variant = "underline", className = "" }) {
    const [inner, setInner] = React.useState(defaultValue || (items[0] && items[0].id));
    const active = value !== undefined ? value : inner;
    const v = VARIANTS.includes(variant) ? variant : "underline";
