...
How an agent applies this system in a TeleDesign project. The project receives a read-only copy at `_ds/technical/`.
...
<link rel="stylesheet" href="_ds/technical/tokens.css">
<link rel="stylesheet" href="_ds/technical/components.css">
```
...
- Components: load after React 18.3.1 / ReactDOM / Babel standalone 7.29 (SRI hashes are in any `*.card.html`):
  `<script type="text/babel" data-presets="react" src="_ds/technical/components/actions/Button.jsx"></script>` — each file sets `window.<Name>`.
...
- Import the library: `"imports": { "ds": "_ds/technical/system.lib.pen" }` and bind properties to its variables (`"fill": "$--primary"`, `"cornerRadius": "$--radius-control"`, `"fontFamily": "$--font-ui"`).
...
- Use mono for anything a machine produced.
- Right-align numerics and use tabular figures.
- Pair every status colour with a word or icon.
- Avoid: No large hero typography.
- Avoid: No decorative colour — every hue means something.
- Avoid: No rounded pills except status dots.
