...
- `llamacpp/{argv,config,state}.py` — build llama-server argv / settings / persist last-active model. `llamacpp/flag_audit.py` — parse `llama-server --help` into a flag spec, snapshot it, cross-check telecode's emitted argv + diff vs a saved snapshot (tray "Flag Audit" card).
...
- **Flag audit** (`flag_audit.py`, on-demand from the tray llama.cpp page's "Flag Audit" card): parses `llama-server --help` into `{flag → {aliases, takes_value, allowed, removed, deprecated}}`, then (1) cross-checks every flag `build_argv` emits across all models — unknown/removed flags + out-of-range enum values — and (2) diffs the live capture against a saved snapshot (removed/added/changed options). Snapshots live in `data/cli-audit/llama-server/` (`baseline.json` + `snapshots/<ts>.json`); `restore(ts)` promotes a snapshot to baseline after backing up the current one. Reports append to `data/logs/cli_audit.log` (in the Logs viewer allowlist). This is the guard that catches flag churn like the v9243 `--checkpoint-every-n-tokens` → `--checkpoint-min-step` rename before it breaks spawn.
