...
Approvals: data/telecode.db approvals (gate | tool | memory) → web inbox badge + Telegram Approve/Reject.
```
...
- `approvals` rows `{kind gate|tool|memory, run_id, step_id, title, body, payload, status pending|approved|rejected|cancelled, decided_*, edited_text, telegram}`; `decide()` is an atomic pending→decided UPDATE (web and Telegram can't both win; the loser gets 409 / "already approved"), then runs the kind's handler (gate → `executor._on_gate_decided`). Pending rows survive restarts. Only `gate` has a producer today.
