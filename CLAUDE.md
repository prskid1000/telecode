...
- **Structured output** (`schema` param): Claude `--json-schema` (result `structured_output`), Codex `--output-schema <tmp file>` (last message parsed as JSON), agy none yet (logged, ignored). Returned as `result.structured_output` when a schema was given.
...
- `sessions_index` (`sessions_repo`): one row per CLI conversation, scoped like resume ids (namespace, workspace, agent, engine, local), with cumulative tokens/cost, `runs_count`, `kind` (task/run/routine/heartbeat/design) and `parent_id` → the conversation it superseded.
