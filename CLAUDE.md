...
Agent → Job → Run (Team Mode):
  ...
  --append-system-prompt-file (Claude) / AGENTS.md (Codex, agy); on exit three-way-merges back, restores.
...
Heartbeat (Team Mode, off by default): parse HEARTBEAT.md → kind="heartbeat" Jobs → fire due+enabled.
  Entry: engine claude_code|codex|antigravity, optional model, is_local. A never-fired entry anchors on its
  persisted first_seen, so it fires at the first cron slot after the scheduler first saw it.
...
- `services/run/executor.py` — pipeline driver per Run. Single-step phase = job workspace; multi-step = ephemeral session per step. Threads outputs via `<previous_output(s)>`: the full reply capped at 16 KB (head + tail around an omission marker) plus `<files_changed>` from a before/after scan of the step's workspace. Steps record `engine/model/is_local`, `result_text`, `files_changed`, `usage`; the run rolls `usage` up (`cost_complete=false` when an engine reports no cost). `reconcile_orphaned_runs()` (called from `api_runs.register_routes`) marks runs/steps left running by a previous process `interrupted`; `cancel_run` also finishes such orphans.
...
- **Fire:** under the routine's `RLock`, re-reading the record: skip-if-running (PENDING/RUNNING → record skipped, `next_fire_at` still advances), and a manager fire re-checks `is_due` so a concurrent run-now cannot double-fire. Heartbeat preface prepended (tick #, cadence, time-since-last-fire, "recurring wake-up — build on prior work"). `outputs_only` appends an output-rule line to the prompt — it is never a handler kwarg (the shared signature would TypeError). Submits via `task_manager.submit_task(..., session_id=rec["session_id"], pool="background")`; `task_timeout_seconds` (default 1800) is enforced. Handlers resume the same session.
- **task_type** is validated on create/patch (`routine_store.normalize_task_type`: `CLAUDE_CODE|CODEX|ANTIGRAVITY`, engine names accepted and upper-cased).
- **Restart:** `routine_manager.start()` marks a routine whose `last_task_id` is gone from the queue as completion status `interrupted`.
...
Three CLIs are dispatched as task types, all sharing the **same handler signature** (`prompt, is_local, agent_id, agent, job, agent_files, job_files, model`) and the same `_agent_task_schema`. The executor / heartbeat scheduler / routine manager are engine-agnostic — they pick the task_type via the single shared map in `services/task/engine_map.py`:
...
- **Codex** — `codex exec [-c …] --sandbox danger-full-access -C <dir> [resume <SID>] --json --dangerously-bypass-approvals-and-sandbox --skip-git-repo-check --output-last-message <path> [--model <m>] -` (prompt on **stdin**, verified on codex-cli 0.157). `--sandbox` and `-C` are `exec`-only and must precede `resume`, which rejects them. Resume id from `thread.started.thread_id`. Usage: `turn.completed.usage` is `{input_tokens, cached_input_tokens, output_tokens, reasoning_output_tokens[, cache_write_input_tokens]}` — **`cached_input_tokens` is a subset of `input_tokens`** (the handler maps `tokens.input` = non-cached, like Claude); summed across `turn.completed` events, `num_turns` = their count, `duration_ms` = wall clock, `cost_usd` = `None` (Codex reports no cost). Staging bridge: `AGENT.md ↔ AGENTS.md`. Event mapping (0.157 names, old spellings kept): `item.completed{agent_message} → narrative`; `item.completed{error} → warning`; `item.completed{command_execution,file_change,mcp_tool_call,web_search} → tool`; `turn.completed.usage → done`; final text from `--output-last-message`.
...
**Resume ids are scoped per (workspace, agent, engine[, local])** — `session.data["resume"]["<agent_id|_>:<engine>[:local]"]` (`task_utils.resume_scope_key` / `read_resume_id` / `make_resume_store`), so pipeline step B never resumes step A's conversation. Runs without an agent fall back to (and keep writing) the legacy flat keys `last_claude_session_id` / `last_codex_session_id` / `last_antigravity[_local]_conversation_id`. TeleDesign calls `_run_*_subprocess` directly without `resume_store` and keeps using the legacy keys.
...
**Subprocess lifecycle.** All three handlers spawn `subprocess.Popen(..., shell=True, creationflags=CREATE_NO_WINDOW)` and then call `task_utils.track_process(proc)`: the shell pid is bound to `process.py`'s kill-on-close Job and registered on the task, so cancel (API, run executor, TeleDesign's queue path) and the timeout watchdog tree-kill it with `process.kill_process_tree` (`terminate()` alone only reached `cmd.exe`). Remaining gap: the CLI the shell starts in the first milliseconds, before the bind, is not in the Job — a hard kill of telecode in that window can still orphan it. P1 (Engine Runner) spawns through `process.py` directly.
