...
9. **Tool-result rewriters** (`proxy/tool_result_rewriters.py`, gated by `proxy.tool_result_rewriting`): generic framework that walks `body.messages[]` on each /v1/messages request, finds `tool_result` blocks whose originating `tool_use` was for a registered tool, and lets the rewriter substitute new content. Drop-in modules live under `proxy/rewriters/`; the framework imports the package on first load (`pkgutil.iter_modules` walk in `proxy/rewriters/__init__.py`) so each module's `register()` call fires automatically — no edits needed to `server.py` or the framework module itself when adding a new rewriter. All rewriters that fire on a single pass run concurrently via `asyncio.gather`. The CC tool_use call stays visible in the CC UI; only the conversation history that the local model sees is modified.
   ...
   - **Engine selection** (verified empirically against the live fork in April 2026): only `bing` is in the default engine list. All other scraping engines fail in known ways from a single residential IP: `google` → CAPTCHA wall (HTTP 302 to `/sorry/index`), `duckduckgo` → CAPTCHA (upstream fix PR #3955 landed after the fork's snapshot date), `mojeek` → HTTP 403 + 24h ban, `qwant` → JSON parser fails on returned HTML CAPTCHA page, `brave` and `yahoo` → silent 0-result returns (HTML parsers are out of sync with the current upstream sites; nothing is logged because the parser "succeeds"), `startpage` → works initially but flaky after a few hours, `wikipedia` → only works on exact title matches. Default kept tight (`["bing"]`) to avoid spamming `unresponsive_engines` with predictable failures and to avoid burning the per-query timeout on engines that will never respond. Users can override via `proxy.web_search.searxng.engines` to add anything from upstream's 254-engine catalog.
