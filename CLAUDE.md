...
10. **Intercept handlers** — five branches in `_handle_streaming`, each produces a `status_line` + `result_content` pair:
    ...
    - **Hallucination guard** — currently NOT wired into the refactored intercept loop (was in the pre-live-status version). Unknown names fall through to the client (CC surfaces its own "no such tool" error). Tracked as an acceptable degradation; re-adding requires wildcard intercept support in `_forward_stream` (treat any name not in known set as intercept).
