...
- **Settings (`settings.docgraph.*`):** `binary`, `host.{enabled, auto_start, auto_restart, host, port, gpu, debounce}`, `roots:[{path, watch, pinned}]`, `llm.*`, `embeddings.{model, gpu, torch_compile, idle_unload_sec}`, `rerank.{default, model, gpu, torch_compile, idle_unload_sec}`, `index.{workers, embed_batch_size}`, `wiki.depth`. Adding/removing roots or flipping `watch` auto-restarts the host. Pinned roots render 📌 and are read-only end-to-end.
- **VRAM reaper (`HostSupervisor._vram_reaper_loop`).** Runs alongside the auto-restart loop. Every 15 s polls `/api/admin/models_status`; when at least one pooled model has been instantiated AND every embed/rerank entry reports `loaded=false` AND no Index/Wiki run is active, `kill_process_tree`s the host. The auto-restart loop then respawns it within ~2 s with a fresh CUDA context (~300 MB freed — `torch.cuda.empty_cache()` can't release the context, only weights). Implicit opt-in: a positive `embeddings.idle_unload_sec` or `rerank.idle_unload_sec` is the gate; without those the host's unloader never fires and the predicate never trips. Gated also on `host.auto_restart` — killing without respawn would be destructive. `_stop_locked` skips the cancel of any task that calls it, so the respawn isn't aborted by self-cancellation.
...
- **Win hybrid graphics (`_ensure_high_perf_gpu`).** Writes `HKCU\Software\Microsoft\DirectX\UserGpuPreferences\<docgraph.exe> = "GpuPreference=2;"` before spawn. Defensive holdover from the ONNX/DirectML era — torch + CUDA enumerates through the NVIDIA driver, not DXGI, so this doesn't gate device choice for the embedder/reranker today. Kept because it's idempotent and protects any DXGI-driven library in-process.
