...
9. **Tool-result rewriters** (`proxy/tool_result_rewriters.py`, gated by `proxy.tool_result_rewriting`): generic framework that walks `body.messages[]` on each /v1/messages request, finds `tool_result` blocks whose originating `tool_use` was for a registered tool, and lets the rewriter substitute new content. Drop-in modules live under `proxy/rewriters/`; the framework imports the package on first load (`pkgutil.iter_modules` walk in `proxy/rewriters/__init__.py`) so each module's `register()` call fires automatically — no edits needed to `server.py` or the framework module itself when adding a new rewriter. All rewriters that fire on a single pass run concurrently via `asyncio.gather`. The CC tool_use call stays visible in the CC UI; only the conversation history that the local model sees is modified.
   - **API**: a rewriter is anything that satisfies the `ToolResultRewriter` Protocol (`tool_name: str`, `should_replace(original) -> bool`, `async replace(tool_input, original) -> Any`). For the common case where you don't need state, the `make_rewriter(tool_name, should_replace, replace)` factory wraps two plain functions and registers them in one call.
   - **File naming convention**: name the rewriter file after the tool it handles, snake-cased (`web_search.py` for `WebSearch`, `web_fetch.py` for `WebFetch`). The match isn't enforced — only the `tool_name` declared in the registration call is — but the convention makes it trivial to find which file handles which tool. MCP tool names (`mcp__plugin_chrome-devtools-mcp_chrome-devtools__click`) can't be filenames, so those just pick a short descriptive name.
   - **`rewriters/web_search.py`**: targets the `WebSearch` tool. CC's executor returns an empty `Web search results for query: "..."` placeholder when no Anthropic backend is reachable; the rewriter detects that fingerprint (starts with the literal prefix and contains no `http://` or `https://`) and queries a local SearXNG instance via `GET {web_search.url}/search?q=...&format=json`. SearXNG dedupes its own results across the underlying engines, but we re-dedupe by URL and trim to `max_results`. Output is rebuilt in the Anthropic-style format with `[N] Title / URL / Snippet / Engine` blocks plus an optional `Summary` line from SearXNG's `answers` field (Wikipedia infoboxes etc.). Caches results by `max_results:query` in an LRU (cap 256). Errors land in the result string as `ERROR: ...` so the model keeps moving instead of retrying.
   - **SearXNG auto-setup** (`ensure_searxng_running()`, no separate flag — opting into the `searxng` provider opts you into setup): called from `start_proxy_background()` before binding the listen socket. **Native install, no .exe / no Docker / no WSL**: clones [`mbaozi/SearXNGforWindows`](https://github.com/mbaozi/SearXNGforWindows) (a Windows-friendly fork of upstream SearXNG) into `data/searxng/repo/` (`git clone --depth 1`), creates a venv at `data/searxng/.venv/`, runs `pip install -r config/requirements.txt`, **copies** the patched `searx/` package out of `repo/python/Lib/site-packages/searx/` into the venv's own `Lib/site-packages/searx/` (so `import searx` resolves to the patched source while compiled deps like msgspec/lxml come from the venv with matching ABI — putting the embedded fork's whole site-packages on PYTHONPATH instead would shadow venv-installed extensions with binaries built for Python 3.11.9 and ImportError on any other version), then generates `data/searxng/config/settings.yml` by loading the upstream template at `repo/config/settings.yml` and overlaying values from `proxy.web_search.searxng.*` in telecode's settings.json. Sibling files in `repo/config/` (e.g. `limiter.toml`) are mirrored into `data/searxng/config/` once. The Telecode-controlled overrides are: `server.bind_address` + `server.port` (parsed from `web_search.url`), `server.secret_key` (random hex on first run if the template's is missing/`ultrasecretkey`), `server.limiter`/`public_instance` = false, `search.safe_search`/`search.default_lang`, `search.formats` (forces `json` in), and the engine list — the upstream fork ships with only Sogou + Baidu + Bing enabled (Chinese-language defaults), and we toggle `disabled` on every engine so only the names listed in `proxy.web_search.searxng.engines` end up enabled. The generated `settings.yml` lives at `data/searxng/config/settings.yml` (NOT inside `repo/`) so `git pull` doesn't clobber it; we also set `SEARXNG_SETTINGS_PATH` to it for explicitness even though the cwd-relative default loader would find it.
   - **Run command**: `.venv/Scripts/python.exe -m searx.webapp` with `cwd=data/searxng/` (one of the upstream fork's patches makes `searx/data/__init__.py` compute `Path(__file__).parent.relative_to(Path.cwd())`, so cwd MUST be an ancestor of `.venv/Lib/site-packages/searx/data/`; `data/searxng/` is the cleanest common ancestor of both `.venv/` and `repo/`). Settings come via `SEARXNG_SETTINGS_PATH` env var. **No PYTHONPATH override** — `searx` is in the venv's own site-packages.
   - **Process lifecycle (Job Object + orphan recovery, 3 layers of safety)**: spawned via `subprocess.Popen` with `CREATE_NO_WINDOW + STARTUPINFO.SW_HIDE` (silent — no console window). The new process is then bound to a Windows **Job Object** with `JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE` via `ctypes` (`CreateJobObjectW` → `SetInformationJobObject` → `AssignProcessToJobObject` → handle held in module-global `_searxng_job_handle`). When Telecode dies for any reason — graceful shutdown, `Stop-ScheduledTask`, `taskkill /F`, BSOD — the kernel closes the job handle and Windows kills every process bound to the job. Layer 2: PID is written to `data/searxng/searxng.pid` after spawn; layer 3: `_kill_orphan()` runs at the start of every `ensure_searxng_running()` call and (a) reads the pid file and kills it via `taskkill /F /T /PID`, (b) probes whichever process is currently listening on `web_search.url`'s port via `Get-NetTCPConnection -LocalPort N` and kills it. So even if the job binding fails or someone restarted SearXNG outside Telecode, we never end up with a stale instance squatting on the port. NB: deliberately NOT using `CREATE_NEW_PROCESS_GROUP` — that flag puts the child in a separate group, breaking job-based parent-death propagation. The graceful `stop_searxng()` path (via `main.py:_post_shutdown`) runs `terminate()` → wait 5s → `kill()` and clears the pid file. `ensure_searxng_running()` always regenerates settings.yml + respawns rather than short-circuiting on "already reachable", so config changes always take effect.
   - **Engine selection** (verified empirically against the live fork in April 2026 by enabling all 25+ candidates and probing each via shortcut bang): the default set picks **one best engine per distinct purpose** — no duplicates — `startpage` (general web), `bing news` (current news), `wikipedia` (encyclopedic facts; results + infoboxes), `wiktionary` (definitions), `reddit` (discussion), `stackoverflow` (programming Q&A), `github` (code repos), `mdn` (web/JS docs), `semantic scholar` (academic papers, indexes arxiv + biomed). Notable choices: `startpage` was chosen over `bing` because bing serves query-specific decoy spam (cryptocurrency blogs, generic Hotels.com pages, Fiverr ads) to SearXNG-IP scrapers — verified side-by-side, `bing` returned 1 unique domain on `2026 midterm elections governor` while `startpage` returned 10 diverse including the exact Wikipedia article. `bing news` is kept because the news module doesn't suffer the same decoy issue. Excluded as duplicates: `wikidata`/`wikinews`/`wikiquote`/`gitlab`/`arxiv`/`hackernews`. Excluded as broken from a residential IP (with logged failure modes): `google` (HTTP 302 to `/sorry/index` CAPTCHA), `duckduckgo` (CAPTCHA — upstream PR #3955 fix postdates the fork's 2025-05 snapshot), `mojeek` (HTTP 403 + 24h SearXNG circuit-breaker ban), `qwant` (HTML CAPTCHA page → JSONDecodeError), `brave`/`yahoo` (silent 0-results — HTML parsers out of sync with current upstream markup), `pypi` (0 results), `pubmed`/`openstreetmap` (timeout), `openlibrary`/`mwmbl` (crash), `reuters` (HTTP error). **Category fan-out caveat**: SearXNG only routes queries to engines whose `categories` field matches the request. Default request category is `general`, so general queries only hit `startpage` + `wikipedia` + `wiktionary`. The specialized engines (`reddit`, `stackoverflow`, `github`, `mdn`, `semantic scholar`, `bing news`) sit in `it`/`news`/`social media` categories and only fire when the model uses the `!shortcut` bang (`!gh flask`, `!st python`, `!bin trump tariffs`).
   - **Result formatter** (`_format_results()`): surfaces three distinct content channels from SearXNG's response — `results` (the standard ranked link list), `infoboxes` (structured per-entity data, e.g. Wikipedia/Wikidata for queries like "Albert Einstein"), and `answers` (one-line summaries from currency conversion / quick facts). Some engines populate only one channel for some queries — e.g. `currency` returns its conversion via `answers` with empty `results`, `wikipedia` returns entity data via `infoboxes` for "Albert Einstein" with empty `results`. Surfacing all three means those engines aren't silently dropped. The `search()` orchestrator treats a response as "no results" only if all three channels are empty.
   - **Failure handling**: every step logs and continues — Telecode still starts; the rewriter just returns ERROR strings until the user fixes the install. Requires `git` on PATH (Windows install: https://git-scm.com/download/win). PyYAML is in `requirements.txt`. On Linux/Mac the same flow runs (uses `bin/python` instead of `Scripts/python.exe`); the fork's source is platform-agnostic except for the embedded Python binaries we ignore. Job Objects are Windows-only — on Linux/Mac the orphan-recovery PID file path is the only safety net.
   - **CC display caveat**: CC's "Did N searches in Xs" count is computed by CC's own WebSearch executor before the empty result ever leaves CC, so the proxy can't reach it — the UI will still show "0 searches" even though the model now sees populated results. The model's text reply will contain real info regardless.
   - **Adding a rewriter**: drop a file at `proxy/rewriters/<tool_name>.py`. Minimum body:
     ```python
     from proxy.tool_result_rewriters import make_rewriter

     def _should(original): return ...   # cheap, sync
     async def _replace(tool_input, _original): return ...  # may do I/O
     make_rewriter("WebFetch", _should, _replace)
     ```
     No edits to `server.py`, `tool_result_rewriters.py`, or anywhere else.
