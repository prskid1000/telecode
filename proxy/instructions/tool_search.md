...
1. **Never call a deferred tool directly.** A name in the deferred list is an identifier, not a loaded tool.
...
4. **Prefer dedicated tools over `web_search`.** If the deferred list contains a domain tool (e.g. MySQL), load and use it instead of web searching.
