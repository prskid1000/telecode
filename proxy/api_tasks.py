...
async def list_types(request: web.Request) -> web.Response:
    return web.json_response(get_task_queue().get_available_task_types())
...
async def submit_task(request: web.Request) -> web.Response:
    ...
    try:
        ...
        return web.json_response({"success": True, "task_id": task_id, "task": task_to_dict(task) if task else None})
...
async def get_task_status(request: web.Request) -> web.Response:
    ...
    return web.json_response({"success": True, **task_to_dict(task)})
...
async def cancel_task(request: web.Request) -> web.Response:
    ...
    # We need to implement cancel_task in manager if not there, for now just update status
    with queue.lock:
    ...
    return web.json_response({"success": True, "task": task_to_dict(task)})
